...
const ApartmentDetails = () => {
  ...
  // init store
  const { getApartmentDetails, apartmentDetailsResponse, apartmentDetailsLoading } = useUnitsStore();

  const handleShare = () => {
    console.log(apartmentDetailsResponse)
  }
          ...
          <View>
            <View className="px-4">
              <Text className="text-xl font-psemibold">
                {apartmentDetailsResponse?.sector?.code?.name} - {apartmentDetailsResponse?.sector?.code?.view_code}
              </Text>
              <Text className="text-sm font-pregular text-zinc-600">
                {apartmentDetailsResponse?.region?.name} - {apartmentDetailsResponse?.post_type == 'share' ? 'أسهم تنظيمية' : 'عقارات'}
              </Text>
            </View>
          </View>
          <ScrollView>
            <View className="flex-1">
              {apartmentDetailsResponse && <CustomImageSlider images={apartmentDetailsResponse?.photos} height={300} newImages={apartmentDetailsResponse} />}
              <View className="mt-4">
              </View>
            </View>
          </ScrollView>
